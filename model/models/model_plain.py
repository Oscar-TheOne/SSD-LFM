...
from models.loss import SparseLoss, VggLoss, CharbonnierLoss, SSIMLoss, CharbonnierEdgeLoss, PerceptualLoss, Mix_1, HybridLoss, EdgeLoss
...
class ModelPlain(ModelBase):
    ...
    def save_general(self, save_dir, save_filename, save_format='h5'):
        self.save_network_general(self.netG, save_dir, save_filename, save_format)
    ...
    def define_loss(self):
        ...
        elif  G_lossfn_type[0:6] in ['hybrid']:
           self.G_lossfn = HybridLoss(10.0 if G_lossfn_type[6]=='x' else float(G_lossfn_type[6])).to(self.device)
        ...
        elif  G_lossfn_type in ['mix1','Mix1']:
           self.G_lossfn = Mix_1().to(self.device)
    ...
    # feed L/H data
    # ----------------------------------------
    def feed_data(self, data, need_H=True, need_G=True, need_R = False, test_patch_size=-1, test_patch_range=[], test_z_range=[]):
        if test_patch_size < 0:
                ...
                if test_z_range:
                    ...
                    if need_H:
                        self.H = data['H'][...,test_z_range[0]:test_z_range[1],test_patch_range[0]:test_patch_range[1],
                                       test_patch_range[2]:test_patch_range[3]].to(self.device)
                    if need_G:
                        self.G = data['G'][...,test_z_range[0]:test_z_range[1],test_patch_range[0]:test_patch_range[1],
                                       test_patch_range[2]:test_patch_range[3]].to(self.device)
                    if need_R:
                    ...
                    if need_H:
                        self.H = data['H'][...,test_patch_range[0]:test_patch_range[1],
                                       test_patch_range[2]:test_patch_range[3]].to(self.device)
                    if need_G:
                        self.G = data['G'][...,test_patch_range[0]:test_patch_range[1],
                                       test_patch_range[2]:test_patch_range[3]].to(self.device)
                    if need_R:
            ...
            if need_H:
                self.H = crop_center(data['H'],test_patch_size)[...,test_patch_range[0]:test_patch_range[1],
                               test_patch_range[2]:test_patch_range[3]].to(self.device)
            if need_G:
                self.G = crop_center(data['G'],test_patch_size)[...,test_patch_range[0]:test_patch_range[1],
                               test_patch_range[2]:test_patch_range[3]].to(self.device)
            if need_R:
            ...
            if need_G:
                self.G = crop_center(data['G'],test_patch_size).to(self.device)
            if need_R:
    ...
    # ----------------------------------------
    # test for 32x data&net
    # ----------------------------------------
    def test_32x(self):
        self.netG.eval()
        if self.L.shape[-1] % 32 == 0:
            padding_size_left = 0
            padding_size_right = 0
        else:
            padding_size_left = (self.L.shape[-1] // 32 * 32 + 32 - self.L.shape[-1]) // 2
            padding_size_right = (self.L.shape[-1] // 32 * 32 + 32 - self.L.shape[-1]) - padding_size_left
        if self.L.shape[-2] % 32 == 0:
            padding_size_up = 0
            padding_size_down = 0
        else:
            padding_size_up = (self.L.shape[-2] // 32 * 32 + 32 - self.L.shape[-2]) // 2
            padding_size_down = (self.L.shape[-2] // 32 * 32 + 32 - self.L.shape[-2]) - padding_size_up
        pad = nn.ZeroPad2d(padding=(padding_size_left, padding_size_right, padding_size_up, padding_size_down))
        temp = pad(self.L)
        with torch.no_grad():
            temp_ = self.netG(temp)
            self.E = temp_[..., padding_size_up:temp_.shape[-2]-padding_size_down, padding_size_left:temp_.shape[-1]-padding_size_right]
        self.netG.train()
    ...
    # get L, E, H image
    # ----------------------------------------
    def current_visuals(self, need_H=True, need_G=True, need_R=False):
        ...
        if need_G:
            out_dict['G'] = self.G.detach()[0].float().cpu()
        if need_R:
    ...
    # ----------------------------------------
    def current_results(self, need_H=True, need_G=True, need_R=False):
        out_dict = OrderedDict()
        ...
        if need_G:
            out_dict['G'] = self.G.detach().float().cpu()
        if need_R:
